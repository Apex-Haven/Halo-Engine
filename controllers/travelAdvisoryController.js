  ...
  try {
    ...
    try {
      preference.recommendations = result.recommendations.map(rec => {
        ...
        // Get the hotel object from the recommendation
        // Priority: rec.hotel (full object), then rec.hotelId if it's an object
        const hotelObj = rec.hotel || (rec.hotelId && typeof rec.hotelId === 'object' ? rec.hotelId : null);
        const hotelIdValue = rec.hotelId && typeof rec.hotelId === 'string' ? rec.hotelId : (hotelObj?.hotelId || hotelObj?._id);
        
        // Only set hotelId if it's a valid MongoDB ObjectId
        if (hotelIdValue && typeof hotelIdValue === 'string' && require('mongoose').Types.ObjectId.isValid(hotelIdValue)) {
          recData.hotelId = hotelIdValue;
          // Also store full hotel data if available
          if (hotelObj) {
            recData.hotelData = hotelObj;
          }
        } else {
          // Store hotel data directly if not a valid ObjectId (for mock/external hotels)
          // Always store the full hotel object if available
          if (hotelObj) {
            recData.hotelData = hotelObj;
          }
          // Store the hotelId string for reference (e.g., "HTL17657088030341")
          if (hotelIdValue && typeof hotelIdValue === 'string') {
            recData.hotelIdString = hotelIdValue;
          }
        }
      ...
      console.log('📊 Sample saved recommendation:', JSON.stringify(preference.recommendations[0] || {}, null, 2).substring(0, 300));
    } catch (saveError) {
  ...
  try {
    ...
    const enhancedRecommendations = await Promise.all(
      (preference.recommendations || []).map(async (rec) => {
        ...
        // Get hotel data - priority: hotelData (stored), hotelId (populated), then try to fetch from DB
        let hotel = null;
        
        // First priority: hotelData (stored full hotel object for mock/external hotels)
        if (recObj.hotelData) {
          hotel = recObj.hotelData;
          // Ensure hotelData is a plain object, not a Mongoose document
          if (hotel && typeof hotel.toObject === 'function') {
            hotel = hotel.toObject();
          }
        } 
        // Second priority: hotelId populated (if it's an object, it's been populated)
        else if (recObj.hotelId && typeof recObj.hotelId === 'object') {
        ...
        // Third priority: hotelId is a string/ID, try to get hotel from database
        ...
        // If still no hotel and we have hotelIdString, log it for debugging
        if (!hotel && recObj.hotelIdString) {
          console.warn('⚠️ Hotel data not found for hotelIdString:', recObj.hotelIdString);
        }
        
        // Debug logging
        if (!hotel) {
          console.warn('⚠️ No hotel data found for recommendation:', {
            hasHotelId: !!recObj.hotelId,
            hasHotelData: !!recObj.hotelData,
            hotelIdType: typeof recObj.hotelId,
            hotelIdValue: recObj.hotelId
          });
        } else {
          console.log('✅ Found hotel data for recommendation:', hotel.name || hotel.hotelId || 'Unknown');
        }
