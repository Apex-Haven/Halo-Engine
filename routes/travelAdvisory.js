...
router.get('/test/:preferenceId', authorize(['SUPER_ADMIN', 'ADMIN', 'OPERATIONS_MANAGER']), async (req, res) => {
  try {
    ...
    const testResults = {
      ...
      dataSources: {
        xotelo: {
          enabled: process.env.XOTELO_ENABLED !== 'false',
          hasApiKey: !!process.env.XOTELO_API_KEY,
          status: 'not_tested'
        },
        hotelSearchService: {
          status: 'not_tested'
    ...
    res.json({
      ...
      environment: {
        xoteloEnabled: process.env.XOTELO_ENABLED,
        xoteloHasApiKey: !!process.env.XOTELO_API_KEY
      }
    });
  } catch (error) {
